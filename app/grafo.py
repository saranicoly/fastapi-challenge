...
class Graph():
  ...
  def add_node(self, person):
    ...
    for friend in person.friends:
      if friend not in self.matriz_adj:
        raise HTTPException(status_code=404, detail=f"Friend {friend} not found. Try adding it first!")
  ...
  def check_person(self, name):
    # Checks if a person exists in the graph
    if name not in self.matriz_adj:
      raise HTTPException(status_code=404, detail=f"Person {name} not found. Try again!")